...
void Verifier::visitInstruction(Instruction &I) {
  ...
  if (MDNode *Noundef = I.getMetadata(LLVMContext::MD_noundef)) {
    Check(isa<LoadInst>(I),
          "noundef applies only to load instructions, use attributes"
          " for calls or invokes",
          &I);
    Check(Noundef->getNumOperands() == 0, "noundef metadata must be empty", &I);
  }
